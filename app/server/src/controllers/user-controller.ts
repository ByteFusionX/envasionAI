...
export const addUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (userExist) {
            return res.send({ message: 'User Exists', status: false })
        } else {
            const nameExist = await userModel.findOne({ name: name })
            if (nameExist) {
                return res.send({ message: 'Username Exists', status: false })
            } else {
                ...
                return res.send({ message: 'User Saved', status: true })
    ...
    try {
        ...
        if (user) {
            ...
            if (hashPassword) {
                ...
                return res.send({ incorrectPassword: true })
            }
        }
        else {
            return res.send({ userExistError: true })
        }
    } catch (error) {
        next(error)
    }
